...
/**
 * 
 *  Implements the class FormulationMasterTimeDecomposition2pmin
*/
...
FormulationMasterTimeDecomposition2pmin::FormulationMasterTimeDecomposition2pmin( InstanceUCP* instance, SCIP* scip_master):
...
FormulationMasterTimeDecomposition2pmin::~FormulationMasterTimeDecomposition2pmin()
{
...
SCIP_RETCODE FormulationMasterTimeDecomposition2pmin::create_variables()
{
...
SCIP_RETCODE FormulationMasterTimeDecomposition2pmin::create_constraints()
{
    ...
    for( int i_unit = 0; i_unit < number_of_units; i_unit ++)
    {
        vector< SCIP_CONS*> cons_pmaxpmin_i;
        for(int i_time_step = 0; i_time_step < number_of_time_steps; i_time_step ++)
        {
            SCIP_CONS* cons_pmaxpmin_i_t;
            current_cons_name.str("");
            current_cons_name << "cons_pmaxpmin_" << i_unit << "_" << i_time_step;

            // create the constraint
            SCIP_CALL( SCIPcreateConsLinear( m_scip_master, 
                &cons_pmaxpmin_i_t,                      /** constraint pointer */ 
                current_cons_name.str().c_str(),             /** constraint name */
                0,                                  /** number of variable added */
                nullptr,                            /** array of variable */
                nullptr,                            /** array of coefficient */
            ...
            // add the constraint
            SCIP_CALL( SCIPaddCons(m_scip_master, cons_pmaxpmin_i_t));
            cons_pmaxpmin_i.push_back(cons_pmaxpmin_i_t);
        }
        m_constraints_pmaxpmin.push_back(cons_pmaxpmin_i);
    }
...
SCIP_RETCODE FormulationMasterTimeDecomposition2pmin::create_and_add_first_columns()
{
    ...
    for( int i_time_step = 0; i_time_step < number_of_time_steps; i_time_step ++)
    {
        ...
        for( int i_unit = 0; i_unit < number_of_units; i_unit++ )
        {
            ...
            quantity_plan[i_unit][i_time_step] = production_min[i_unit];
...
SCIP_RETCODE FormulationMasterTimeDecomposition2pmin::add_column( ProductionPlan* plan_of_new_column, bool initialization, int block_number ) 
{
    int number_of_time_steps( m_instance_ucp->get_time_steps_number() );
    int number_of_units( m_instance_ucp->get_units_number());
    ...
    for( int i_unit = 0; i_unit < number_of_units; i_unit++ )
    {
        // pmaxpmin
        SCIP_CALL( SCIPaddCoefLinear(m_scip_master,
            m_constraints_pmaxpmin[i_unit][block_number],
            new_scip_variable,                                          /* variable to add */
            + ( production_max[i_unit] - production_min[i_unit] ) * up_down_plan[i_unit][block_number] ));       /* coefficient */
    }
    ...
    {
        SCIP_CALL( SCIPaddCoefLinear(m_scip_master,
            m_constraints_demand[block_number],
            new_scip_variable,                                          /* variable to add */
...
ProductionPlan* FormulationMasterTimeDecomposition2pmin::get_production_plan_from_solution()
{
    ...
    for(int i_column = 0; i_column < number_columns; i_column ++)
    {
        ...
        for( int i_unit = 0; i_unit < number_of_units; i_unit++ )
        {
            ...
            quantity_plan[i_unit][current_block] += current_up_down_plan[i_unit][current_block]*coefficient_column * production_min[i_unit];
        }
    }
...
SCIP_CONS** FormulationMasterTimeDecomposition2pmin::get_constraint_convexity( int number_block )
{
    return( &m_constraints_convexity[number_block]);
}

SCIP_Cons** FormulationMasterTimeDecomposition2pmin::get_constraint_min_uptime( int number_unit, int number_time_step )
{
    return( &m_constraints_min_uptime[number_unit][number_time_step]);
}

SCIP_Cons** FormulationMasterTimeDecomposition2pmin::get_constraint_min_downtime( int number_unit, int number_time_step )
{
    return( &m_constraints_min_downtime[number_unit][number_time_step]);
}

SCIP_Cons** FormulationMasterTimeDecomposition2pmin::get_constraint_switch( int number_unit, int number_time_step )
{
    return( &m_constraints_switch[number_unit][number_time_step]);
}


SCIP_Cons** FormulationMasterTimeDecomposition2pmin::get_constraint_pmaxpmin( int number_unit, int number_time_step )
{
    return( &m_constraints_pmaxpmin[number_unit][number_time_step]);
}

SCIP_Cons** FormulationMasterTimeDecomposition2pmin::get_constraint_demand( int number_time_step )
{
    return( &m_constraints_demand[number_time_step]);
}



int FormulationMasterTimeDecomposition2pmin::get_columns_number()
{
