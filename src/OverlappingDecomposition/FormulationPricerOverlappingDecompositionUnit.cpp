...
FormulationPricerOverlappingDecompositionUnit::FormulationPricerOverlappingDecompositionUnit(InstanceUCP *instance, 
        SCIP *scip, 
        int block_number,
...
SCIP_RETCODE FormulationPricerOverlappingDecompositionUnit::create_variables()
{
    ...
    // Variables x
    double production_max( m_instance_ucp->get_production_max()[m_block_number]);
    double production_min( m_instance_ucp->get_production_min()[m_block_number]);
    double fixed_cost_unit_i(m_instance_ucp->get_costs_fixed()[m_block_number]);
    ...
    // Variables u
    int cost_start_i(m_instance_ucp->get_costs_startup()[m_block_number]);
...
SCIP_RETCODE FormulationPricerOverlappingDecompositionUnit::create_constraints()
{
    ...
    current_cons_name << "cons_startup_" << m_block_number << "_0";
    ...
    SCIP_CALL(SCIPcreateConsBasicLinear(m_scip_pricer, 
        ...
        +initial_state[m_block_number] ));          /* RHS */
    ...
    for(int i_time_step = 1; i_time_step < number_of_time_steps; i_time_step++)
    {
        ...
        current_cons_name.str("");
        current_cons_name << "cons_startup_" << m_block_number << "_" << i_time_step;
    ...
    for(int i_time_step = min_uptime[m_block_number] - 1; i_time_step < number_of_time_steps; i_time_step ++)
    {
        SCIP_CONS* cons_min_uptime_i_t;
        current_cons_name.str("");
        current_cons_name << "cons_min_uptime_" << m_block_number << "_" << i_time_step;
        ...
        for(int i_time_step2 = i_time_step - min_uptime[m_block_number] + 1 ; i_time_step2 < i_time_step + 1; i_time_step2++)
        {
            SCIP_CALL( SCIPaddCoefLinear( m_scip_pricer,
                cons_min_uptime_i_t,
    ...
    for(int i_time_step = min_downtime[m_block_number]; i_time_step < number_of_time_steps; i_time_step ++)
    {
        SCIP_CONS* cons_min_downtime_i_t;
        current_cons_name.str("");
        current_cons_name << "cons_min_downtime_" << m_block_number << "_" << i_time_step;
        ...
        SCIP_CALL( SCIPaddCoefLinear(m_scip_pricer,
            ...
            m_variable_x[i_time_step - min_downtime[m_block_number]],                           /** variable to add */
            1));                                    /** coefficient */
        for(int i_time_step2 = i_time_step - min_downtime[m_block_number] + 1 ; i_time_step2 < i_time_step + 1; i_time_step2++)
        {
            SCIP_CALL( SCIPaddCoefLinear( m_scip_pricer,
                cons_min_downtime_i_t,
                m_variable_u[i_time_step2],             /** variable to add */
...
SCIP_RETCODE FormulationPricerOverlappingDecompositionUnit::change_reduced_costs(
...
)
{
    ...
    // Variables x
    double fixed_cost_unit_i(m_instance_ucp->get_costs_fixed()[m_block_number]);
    double prop_cost_unit_i( m_instance_ucp->get_costs_proportionnal()[ m_block_number ]); 
    int production_min( m_instance_ucp->get_production_min()[ m_block_number ] );
    ...
    for(int i_time_step = 0; i_time_step < number_of_time_steps; i_time_step ++)
    {
        ...
        double coefficient( 0. );
        coefficient += ( 1 - m_coefficient_repartition_fixed ) * fixed_cost_unit_i;
        coefficient -=  reduced_costs_variable_splitting[ i_time_step ];
        coefficient += m_coefficient_repartition_prop * prop_cost_unit_i * production_min ;
...
ProductionPlan* FormulationPricerOverlappingDecompositionUnit::get_production_plan_from_solution()
{
...
int FormulationPricerOverlappingDecompositionUnit::get_block_number()
{
