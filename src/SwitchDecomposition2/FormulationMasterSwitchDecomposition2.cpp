...
/**
 * 
 *  Implements the class FormulationMasterSwitchDecomposition2
*/
...
FormulationMasterSwitchDecomposition2::FormulationMasterSwitchDecomposition2( 
...
SCIP_RETCODE FormulationMasterSwitchDecomposition2::create_variables()
{
...
SCIP_RETCODE FormulationMasterSwitchDecomposition2::create_constraints()
{
    ...
    for(int i_unit = 0; i_unit < number_of_units; i_unit ++)
    {
        ...
        for(int i_time_step = min_uptime[i_unit] - 1; i_time_step < number_of_time_steps; i_time_step ++)
        {
            ...
            // add the variables : none (only columns)
...
SCIP_RETCODE FormulationMasterSwitchDecomposition2::create_and_add_first_columns()
{
...
SCIP_RETCODE FormulationMasterSwitchDecomposition2::add_column( ProductionPlan* plan_of_new_column, bool initialization, int block_number ) 
{
    int number_of_time_steps( m_instance_ucp->get_time_steps_number() );
    int number_of_units( m_instance_ucp->get_units_number());
    ...
    for( int i_unit = 0; i_unit < number_of_units; i_unit ++)
    {
        ...
        for( int i_time_step = min_uptime[i_unit] - 1; i_time_step < number_of_time_steps; i_time_step ++ )
        {
            if( block_number <= i_time_step )
            {
                if( block_number >= i_time_step - min_uptime[ i_unit ] + 1)
                {
                    SCIP_CALL( SCIPaddCoefLinear(m_scip_master,
                    m_constraints_min_uptime[i_unit][ i_time_step - min_uptime[i_unit] + 1 ],
                    new_scip_variable,        /** variable to add */
                    + switch_plan[i_unit][block_number]                               /** coefficient */
                    ));  
                }
            }
        }
    ...
    for( int i_unit = 0; i_unit < number_of_units; i_unit ++)
    {
        ...
        for( int i_time_step = min_downtime[i_unit]; i_time_step < number_of_time_steps; i_time_step ++ )
        {
            if( block_number >= i_time_step - min_downtime[ i_unit ] + 1)
            {
                if( block_number <= i_time_step)
                {
                    SCIP_CALL( SCIPaddCoefLinear(m_scip_master,
                    m_constraints_min_downtime[i_unit][i_time_step - min_downtime[i_unit] ],
                    new_scip_variable,                                  // variable to add
        ...
        // for( int i_time_step = block_number; i_time_step < block_number + min_downtime[i_unit] - 1 ; i_time_step ++ )
        // {
        //     if( i_time_step >= min_downtime[i_unit] )
        //     {
        //         if( i_time_step < number_of_time_steps )
        //         {
        //             SCIP_CALL( SCIPaddCoefLinear(m_scip_master,
        //                 m_constraints_min_downtime[i_unit][i_time_step - min_downtime[i_unit] ],
        //                 new_scip_variable,                                  // variable to add
...
ProductionPlan* FormulationMasterSwitchDecomposition2::get_production_plan_from_solution()
{
...
int FormulationMasterSwitchDecomposition2::get_columns_number()
{
    return(m_vector_columns.size());
}


SCIP_CONS** FormulationMasterSwitchDecomposition2::get_constraint_convexity( int time_step_number )
{
    return( &m_constraints_convexity[ time_step_number ] );
}

SCIP_CONS** FormulationMasterSwitchDecomposition2::get_constraint_min_uptime( int unit_number, int time_step_number )
{
    return( &m_constraints_min_uptime[ unit_number ][ time_step_number ] );
}

SCIP_CONS** FormulationMasterSwitchDecomposition2::get_constraint_min_downtime( int unit_number, int time_step_number )
{
    return( &m_constraints_min_downtime[ unit_number ][ time_step_number ] );
}

SCIP_CONS** FormulationMasterSwitchDecomposition2::get_constraint_variable_splitting( int unit_number, int time_step_number )
{
